...
def gauss_o2_o3_w_cont(x, z, sig, n1, n21, n3, a1, b1, a2, b2):
    global line, lsf
    global wavemin1, wavemax1
    global wavemin2, wavemax2
    n2 = n1*n21
    n4 = n3*3.
    mask1 = (x>wavemin1) & (x<wavemax1)
    mask2 = (x>wavemin2) & (x<wavemax2)
    ...
    cont[mask1] = cont1
    cont[mask2] = cont2
    g1 = gauss(x, line[0]*(1+z), concolve_lsf(sig, lsf[0])/2.998e5*line[0]*(1+z), n1)
    g2 = gauss(x, line[1]*(1+z), concolve_lsf(sig, lsf[1])/2.998e5*line[1]*(1+z), n2)
    g3 = gauss(x, line[2]*(1+z), concolve_lsf(sig, lsf[2])/2.998e5*line[2]*(1+z), n3)
    g4 = gauss(x, line[3]*(1+z), concolve_lsf(sig, lsf[3])/2.998e5*line[3]*(1+z), n4)
    return g1+g2+g3+g4+cont

def gauss_o2_o3_2comp_w_cont(x, z1, z2, sig1, sig2, n1, n21, n3, n5, n65, n7, a1, b1, a2, b2):
    global line, lsf
    global wavemin1, wavemax1
    global wavemin2, wavemax2
    n2 = n1*n21
    ...
    cont[mask1] = cont1
    cont[mask2] = cont2
    g1 = gauss(x, line[0]*(1+z1), concolve_lsf(sig1, lsf[0])/2.998e5*line[0]*(1+z1), n1)
    g2 = gauss(x, line[1]*(1+z1), concolve_lsf(sig1, lsf[1])/2.998e5*line[1]*(1+z1), n2)
    g3 = gauss(x, line[2]*(1+z1), concolve_lsf(sig1, lsf[2])/2.998e5*line[2]*(1+z1), n3)
    g4 = gauss(x, line[3]*(1+z1), concolve_lsf(sig1, lsf[3])/2.998e5*line[3]*(1+z1), n4)
    g5 = gauss(x, line[0]*(1+z2), concolve_lsf(sig2, lsf[0])/2.998e5*line[0]*(1+z2), n5)
    g6 = gauss(x, line[1]*(1+z2), concolve_lsf(sig2, lsf[1])/2.998e5*line[1]*(1+z2), n6)
    g7 = gauss(x, line[2]*(1+z2), concolve_lsf(sig2, lsf[2])/2.998e5*line[2]*(1+z2), n7)
    g8 = gauss(x, line[3]*(1+z2), concolve_lsf(sig2, lsf[3])/2.998e5*line[3]*(1+z2), n8)

    return g1+g2+g3+g4+g5+g6+g7+g8+cont

class Fitting():
	def __init__(self, cube, wavebounds, p0, plim, xlim, ylim):
	...
	def set_func(self, nfunc):
		if nfunc == 1: self.func = gauss_o2_o3_w_cont
		if nfunc == 2: self.func = gauss_o2_o3_2comp_w_cont
	...
	def dofit(self):
		...
		for y in range(self.ylim[0], self.ylim[1]):
			for x in range(self.xlim[0], self.xlim[1]):
				...
				# 							sigma=errfit, p0=self.p0, bounds=slef.plim)
